...
Moira::reset()
{
    // std::fprintf(stderr, "Moira::reset() dispatcher called. model=%d\\n", (int)cpuModel);
    switch (cpuModel) {
...
Moira::reset()
{
    ...
    // std::fprintf(stderr, "Moira::reset: read16OnReset(0) -> %04x\\n", (unsigned int)reg.sp);
    SYNC(4);
    reg.isp = reg.sp = (read16OnReset(2) & ~0x1) | reg.sp << 16;
    // std::fprintf(stderr, "Moira::reset: SSP set to %08x\\n", (unsigned int)reg.sp);
    SYNC(4);
    // std::fprintf(stderr, "Moira::reset: Fetching PC...\\n");
    reg.pc = read16OnReset(4);
    // std::fprintf(stderr, "Moira::reset: read16OnReset(4) -> %04x\\n", (unsigned int)reg.pc);
    SYNC(4);
    reg.pc = (read16OnReset(6) & ~0x1) | reg.pc << 16;
    // std::fprintf(stderr, "Moira::reset: PC set to %08x\\n", (unsigned int)reg.pc);
...
Moira::execute()
{
    static int count = 0;
    if (count < 100) {
        // std::fprintf(stderr, "Moira::execute: PC=%08x\n", (unsigned int)reg.pc);
        std::fflush(stderr);
        count++;
    }
...
Moira::setIPL(u8 val)
{
    static int count = 0;
    if (count < 100) {
        // std::fprintf(stderr, "Moira::setIPL: val=%d ipl=%d\n", (int)val, (int)ipl);
        std::fflush(stderr);
        count++;
    }
    if (ipl != val) {
